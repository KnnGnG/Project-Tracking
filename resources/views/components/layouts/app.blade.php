...
>
        ...
        <nav @click="if ($event.target.closest('a')) closeSidebar()" class="app-scrollbar flex-1 space-y-1 overflow-y-auto px-4 py-5">
            <?php if ($isAuthed) { ?>
                <?php if ($isAdmin) { ?>
                    ...
                    <a href="<?= e(route('admin.users')) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('admin.users') ? $navActive : $navIdle) ?>">
                    ...
                    <a href="<?= e(route('admin.projects')) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('admin.projects') ? $navActive : $navIdle) ?>">
                    ...
                    <a href="<?= e(route('admin.teams')) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('admin.teams') ? $navActive : $navIdle) ?>">
                    ...
                    <a href="<?= e(route('admin.tasks')) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('admin.tasks') ? $navActive : $navIdle) ?>">
                ...
                <?php } elseif ($isClient) { ?>
                    <p class="<?= e($sectionClass) ?>">Client</p>
                    <a href="<?= e(route('client.dashboard')) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('client.dashboard') ? $navActive : $navIdle) ?>">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        My Projects
                    </a>
                <?php } else { ?>
                    <p class="<?= e($sectionClass) ?>">Projects</p>
                    <a href="<?= e(route('projects.index')) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('projects.*') ? $navActive : $navIdle) ?>">
                    ...
                    <?php if ($showTeamLeadNav) { ?>
                    <p class="<?= e($sectionClass) ?>">Team Lead</p>
                    <a href="<?= e(route('lead.dashboard', $activeLeadRouteParams)) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('lead.dashboard') ? $navActive : $navIdle) ?>">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                        Dashboard
                    </a>
                    <a href="<?= e(route('lead.analytics', $activeLeadRouteParams)) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('lead.analytics') ? $navActive : $navIdle) ?>">
                    ...
                    <a href="<?= e(route('lead.tasks', $activeLeadRouteParams)) ?>" class="<?= e($navClass) ?> <?= e(request()->routeIs('lead.tasks') ? $navActive : $navIdle) ?>">
            ...
            <div class="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-full bg-indigo-500 text-sm font-bold text-white">
                        <?= e(strtoupper(substr($authUser->name, 0, 1))) ?>
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-semibold text-white"><?= e($authUser->name) ?></p>
                        <p class="truncate text-xs font-medium text-slate-400"><?= e($roleLabel) ?></p>
                    </div>
                </div>
                <form method="POST" action="<?= e(route('logout')) ?>" class="mt-3">
                    <?= csrf_field() ?>
    ...
    {{-- Main content --}}
    <main class="{{ $lockMainScroll ? 'flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden' : 'app-scrollbar min-w-0 flex-1 overflow-y-auto' }}" :inert="sidebarOpen && ! isDesktop" :aria-hidden="(sidebarOpen && ! isDesktop).toString()" @keydown.escape.window="closeSidebar()">
        {{-- Top bar --}}
        ...
        @if($lockMainScroll)
            <div class="min-h-0 flex-1 overflow-hidden px-6 py-6 lg:px-8 lg:py-8">
